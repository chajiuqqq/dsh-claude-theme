...
.ctDisguiseIcon{color:var(--dsw-alias-label-primary);pointer-events:none}
.ctDisguiseIcon svg{width:100%;height:100%;display:block}
`;
    //#endregion
    ...
    /** 纯鲸鱼图标（FishLogo，空状态英雄区旁）→ 替换为 Claude 星芒图标（无文字）。 */
    function disguiseFishIcon(svg) {
      const parent = svg.parentElement;
      if (parent === null) return;
      if (disguise.svgOverlay.has(svg)) {
        svg.style.display = "none";
      ...
      const overlay = document.createElement("span");
      overlay.setAttribute(DISGUISE_ATTR, "1");
      overlay.className = "ctDisguiseIcon";
      overlay.style.cssText =
        `display:inline-flex;vertical-align:middle;width:${Math.round(box.width)}px;height:${Math.round(box.height)}px;`;
      overlay.innerHTML =
        `<svg viewBox="0 0 24 24" fill="currentColor" xmlns="http://www.w3.org/2000/svg" aria-hidden="true">${claudeRays(12, 12, 5, 11, 2.6)}</svg>`;
      parent.appendChild(overlay);
      disguise.svgOverlay.set(svg, overlay);
    }
    ...
    function scanAll() {
      ...
      // primitives 里的词标（BrandWordmark）与纯鲸鱼图标（FishLogo）兜底
      for (const svg of document.querySelectorAll('svg[viewBox="0 0 182 24"]')) disguiseWordmark(svg);
      for (const svg of document.querySelectorAll('svg[viewBox="0 0 23.16 17.04"]')) disguiseFishIcon(svg);
      disguiseTitle();
      disguiseFavicons();
    }
